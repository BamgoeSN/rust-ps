...
#[derive(Clone, Debug)]
pub(crate) struct Ntt<const P: u64> {
	pub(crate) arr: Vec<u64>,
}

impl<const P: u64> Ntt<P> {
	pub(crate) const fn ntt_a() -> u64 {
		let mut p = P - 1;
		while p & 1 == 0 {
			p >>= 1;
		}
	...
	pub(crate) fn new(arr: Vec<u64>) -> Self { Self { arr } }
	...
	pub(crate) fn convolute(a: &[u64], b: &[u64]) -> Self {
...
pub(crate) fn convolute(a: &[u64], b: &[u64]) -> Vec<u64> {
